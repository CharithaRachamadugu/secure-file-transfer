...
def decrypt_file(file_path, key):
    ...
    output_file_path = file_path[:-4] # Output file path based on the first four characters of input file path
            ...
            with open('received_encrypted_file.enc', 'wb') as f:
                while True:
                    ...
                    if not data:
                        break
                    f.write(data)
            print("Received encrypted AES key, digital signature, and file from gmailserver.")
...
def main():
    ...
    if verify_signature(encrypted_signature, decrypted_file_hash, sender_public_key):
        print("Digital signature verified. File integrity and authenticity confirmed.")
        # Print the decrypted file
        with open(decrypted_file_path, 'rb') as f:
            print(f"Decrypted file content:\n{f.read().decode()}")
    else:
        print("Digital signature verification failed. The file may have been tampered with.")
